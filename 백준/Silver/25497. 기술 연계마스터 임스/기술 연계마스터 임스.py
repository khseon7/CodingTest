...
pre_skill={'L':0,'S':0} # 사전 기술 및 본 기술 딕셔너리
cnt=0 # 기술 시전 횟수
for i in input_list:
  if i=='L': # 사전 기술 L이 들어오면 한번 카운트해서 저장
    ...
    if pre_skill['L']>0: # 사전 기술 L이 카운트되어 있다면 하나 줄이고, cnt를 하나 더 카운트
      pre_skill['L']-=1
      cnt+=1
    else: # 사전 기술이 없다면 스크립트가 꼬여 후에 기술이 정상적으로 발동되지 않아 끝냄
      break
  elif i=='K': # 본 기술 K이 들어오면
    if pre_skill['S']>0: # 사전 기술 S가 카운트되어 있다면 하나 줄이고, cnt를 하나 더 카운트
      pre_skill['S']-=1
      cnt+=1
